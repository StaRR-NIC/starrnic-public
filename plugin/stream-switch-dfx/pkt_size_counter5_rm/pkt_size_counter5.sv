...
module rm_filler (
  ...
  generate for (genvar i = 0; i < NUM_INTF; i++) begin
    axi_stream_size_counter_rm_counter5 #(
      .TDATA_W (512)
    ) byte_counter_inst (
  ...
  generate for (genvar i = 0; i < NUM_INTF; i++) begin
    ...
    axi_stream_pipeline_rm_counter5 tx_ppl_inst (
    ...
    if (~axis_aresetn) begin
      ...
      case (reg_addr)
        REG_BYTE_COUNT_IF1: begin
          reg_dout[15:0] <= size[15:0] + 5;
          reg_dout[31:16] <= 0;
        end
        REG_BYTE_COUNT_IF2: begin
          reg_dout[15:0] <= size[NUM_INTF*16-16+:16] + 5;
          reg_dout[31:16] <= 0;
        end
        default: begin
