...
class TB:
    def __init__(self, dut):
        ...
        self.source = AxiStreamSource(
            AxiStreamBus.from_prefix(dut, "s_axis"), dut.axis_aclk, dut.axis_aresetn, reset_active_level=False)
        self.sink = AxiStreamSink(
            AxiStreamBus.from_prefix(dut, "m_axis"), dut.axis_aclk, dut.axis_aresetn, reset_active_level=False)
...
async def check_connection(tb, source, sink, test_packet=packets[0]):
    ...
    test_frames = []
    test_frame = AxiStreamFrame(bytes(test_packet))
